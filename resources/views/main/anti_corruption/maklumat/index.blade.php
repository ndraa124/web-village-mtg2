      ...
      <article class="max-w-none text-gray-700">
        ...
        <div class="flex flex-wrap items-center text-gray-600 text-sm gap-4 mb-6">
          <span>
            ...
            {{ $maklumat->user->name ?? 'Admin' }}
          </span>
          <span>
            <i class="far fa-calendar mr-2"></i>
            {{ $maklumat->created_at->format('d F Y') }}
          </span>
          <span>
            <i class="far fa-clock mr-2"></i>
            {{ $maklumat->created_at->format('H:i') }} WITA
          </span>
        </div>
        ...
        <div class="flex items-center gap-3 mb-6">
          @php
            ...
            $shareTitle = urlencode('Pengawasan - Desa Ati Korupsi Motoling Dua');
        ...
        <img src="{{ $maklumat->image_url }}" alt="{{ $maklumat->title }}" class="w-full rounded-lg mb-6">
        <p class="text-center text-sm text-gray-600 italic mb-6"></p>

        <div class="fs-16 text-body news-content">
          @if (empty($maklumat) || $maklumat->content == '')
