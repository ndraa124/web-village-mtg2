<div class="card bg-white rounded-10 border border-white mb-4">
  <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 p-20">
    <form action="{{ route('master.religion.index') }}" method="GET" class="table-src-form position-relative m-0">
    ...
    <a href="{{ route('master.religion.create') }}" class="text-primary fs-16 text-decoration-none">+ Tambah Baru</a>
  </div>
      ...
      <table class="table align-middle">
        <thead>
          <tr>
            ...
            <th scope="col" class="fw-medium text-center">Nama Agama</th>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-body">{{ $row->religion_name }}</td>
            <td class="text-body text-center">{{ $row->created_at->format('d M Y, H:i') }}</td>
            <td>
              <form action="{{ route('master.religion.destroy', $row->id) }}" method="POST">
                <div class="d-flex justify-content-center" style="gap: 18px;">

                  <a href="{{ route('master.religion.show', $row->id) }}" class="bg-transparent p-0 border-0 hover-text-success" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Detail">
                    <i class="material-symbols-outlined fs-16 fw-normal text-primary">visibility</i>
                  </a>

                  <a href="{{ route('master.religion.edit', $row->id) }}" class="bg-transparent p-0 border-0 hover-text-warning" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Edit">
