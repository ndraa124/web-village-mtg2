<div class="card bg-white rounded-10 border border-white mb-4">
  <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 p-20">
    <form action="{{ route('admin.content.slider.index') }}" method="GET" class="table-src-form position-relative m-0">
      <input type="text" name="search" class="form-control w-350" value="{{ request('search') }}" placeholder="Cari judul atau sub judul...">
    ...
    <a href="{{ route('admin.content.slider.create') }}" class="text-primary fs-16 text-decoration-none">+ Tambah Baru</a>
  </div>
        ...
        <tbody>
          @forelse ($sliders as $row)
            <tr>
              ...
              <td>
                <img src="{{ $row->image_url }}" alt="{{ $row->title }}" style="width: 120px; height: 70px; object-fit: cover; border-radius: 8px;">
              </td>
              ...
              <td>
                <form action="{{ route('admin.content.slider.destroy', $row->id) }}" method="POST">
                  <div class="d-flex justify-content-center" style="gap: 18px;">

                    <a href="{{ route('admin.content.slider.show', $row->id) }}" class="bg-transparent p-0 border-0 hover-text-success" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Detail">
                      <i class="material-symbols-outlined fs-16 fw-normal text-primary">visibility</i>
                    </a>

                    <a href="{{ route('admin.content.slider.edit', $row->id) }}" class="bg-transparent p-0 border-0 hover-text-warning" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Edit">
