    ...
    <div class="card bg-white p-20 rounded-10 border border-white mb-4">
      <div class="d-flex gap-4 fs-14 text-secondary mb-2">
        <span>
          ...
          {{ $serviceQuality->user->name ?? 'N/A' }}
        </span>
        <span>
          <i class="material-symbols-outlined fs-16" style="vertical-align: middle; margin-top: -3px;">calendar_month</i>
          Dibuat: {{ $serviceQuality->created_at->format('d M Y') }}
        </span>
      </div>

      <hr>

      <form action="{{ route('admin.content.anti.service-quality.update', $serviceQuality->id) }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')

        <div class="row">
          <div class="col-lg-12">
            <div class="mb-20">
              <label class="label fs-16 mb-2">Konten Kualitas Pelayanan Publik</label>
              <div class="form-group">
                <textarea id="ckeditor-editor" name="content" class="form-control" rows="10" placeholder="Masukkan konten...">{{ old('content', $serviceQuality->content) }}</textarea>
              </div>
              @error('content')
        ...
        <div class="row">
          <div class="col-lg-12">
            <div class="d-flex gap-2">
              ...
              <a href="{{ route('admin.content.anti.service-quality.index') }}" class="btn btn-danger fw-normal text-white">Batal</a>
            </div>
          </div>
        </div>
      </form>
