    ...
    <div class="card bg-white p-20 rounded-10 border border-white mb-4">
      <div class="d-flex gap-4 fs-14 text-secondary mb-2">
        <span>
          ...
          {{ $supervision->user->name ?? 'N/A' }}
        </span>
        <span>
          <i class="material-symbols-outlined fs-16" style="vertical-align: middle; margin-top: -3px;">calendar_month</i>
          Dibuat: {{ $supervision->created_at->format('d M Y') }}
        </span>
      </div>

      <hr>

      <form action="{{ route('admin.content.anti.supervision.update', $supervision->id) }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')
          ...
          <div class="col-lg-12">
            <div class="mb-20">
              ...
              <div class="form-group">
                <textarea id="ckeditor-editor" name="content" class="form-control" rows="10" placeholder="Masukkan konten...">{{ old('content', $supervision->content) }}</textarea>
              </div>
              @error('content')
        ...
        <div class="row">
          <div class="col-lg-12">
            <div class="d-flex gap-2">
              ...
              <a href="{{ route('admin.content.anti.supervision.index') }}" class="btn btn-danger fw-normal text-white">Batal</a>
            </div>
          </div>
        </div>
      </form>
