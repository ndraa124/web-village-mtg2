<div class="card bg-white rounded-10 border border-white mb-4">
  <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 p-20">
    <form action="{{ route('admin.content.galleries.index') }}" method="GET" class="table-src-form position-relative m-0">
      <input type="text" name="search" class="form-control w-350" value="{{ request('search') }}" placeholder="Cari berdasarkan keterangan...">
    ...
    <a href="{{ route('admin.content.galleries.create') }}" class="text-primary fs-16 text-decoration-none">+ Tambah Baru</a>
  </div>
      ...
      <table class="table align-middle">
        ...
        <tbody>
          @forelse ($galleries as $gallery)
            <tr>
              <td class="text-body text-center">{{ $galleries->firstItem() + $loop->index }}</td>
              <td>
                <img src="{{ $gallery->image_url }}" alt="{{ $gallery->caption ?? 'Gallery Image' }}" style="width: 100px; height: 70px; object-fit: cover; border-radius: 8px;">
              </td>
              <td class="text-body">{{ $gallery->caption ?? '-' }}</td>
              <td class="text-body text-center">{{ $gallery->created_at->format('d M Y, H:i') }}</td>
              <td>
                <form action="{{ route('admin.content.galleries.destroy', $gallery->id) }}" method="POST">
                  <div class="d-flex justify-content-center" style="gap: 18px;">

                    <a href="{{ route('admin.content.galleries.edit', $gallery->id) }}" class="bg-transparent p-0 border-0 hover-text-warning" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Edit">
