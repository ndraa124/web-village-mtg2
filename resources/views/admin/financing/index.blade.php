<div class="card bg-white rounded-10 border border-white mb-4">
  <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 p-20">
    <form action="{{ route('master.financing.index') }}" method="GET" class="table-src-form position-relative m-0">
      <input type="text" name="search" class="form-control w-350" value="{{ request('search') }}" placeholder="Cari nama pembiayaan...">
    ...
    <a href="{{ route('master.financing.create') }}" class="text-primary fs-16 text-decoration-none">+ Tambah Baru</a>
  </div>
      ...
      <table class="table align-middle">
        <thead>
          <tr>
            ...
            <th scope="col" class="fw-medium text-center">Nama Pembiayaan</th>
            <th scope="col" class="fw-medium text-center">Tanggal Dibuat</th>
            <th scope="col" class="fw-medium text-center">Aksi</th>
          </tr>
        </thead>
        <tbody>
          @forelse ($financings as $row)
          <tr>
            <td class="text-body text-center">{{ $loop->iteration }}</td>
            <td class="text-body">{{ $row->financing_name }}</td>
            <td class="text-body text-center">{{ $row->created_at->format('d M Y, H:i') }}</td>
            <td>
              <form action="{{ route('master.financing.destroy', $row->id) }}" method="POST">
                <div class="d-flex justify-content-center" style="gap: 18px;">

                  <a href="{{ route('master.financing.show', $row->id) }}" class="bg-transparent p-0 border-0 hover-text-success" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Detail">
                    <i class="material-symbols-outlined fs-16 fw-normal text-primary">visibility</i>
                  </a>

                  <a href="{{ route('master.financing.edit', $row->id) }}" class="bg-transparent p-0 border-0 hover-text-warning" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Edit">
