<div class="card bg-white rounded-10 border border-white mb-4">
  <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 p-20">
    <form action="{{ route('admin.users.index') }}" method="GET" class="table-src-form position-relative m-0">
    ...
    <a href="{{ route('admin.users.create') }}" class="text-primary fs-16 text-decoration-none">+ Tambah Baru</a>
  </div>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-center">
              <h4 class="fw-medium fs-16 mb-0">{{ $row->nik }}</h4>
            </td>
            <td class="text-body">{{ $row->name }}</td>
            ...
            <td>
              <form action="{{ route('admin.users.destroy', $row->id) }}" method="POST">
                <div class="d-flex justify-content-center" style="gap: 18px;">
                  ...
                  <a href="{{ route('admin.users.show', $row->id) }}" class="bg-transparent p-0 border-0 hover-text-success" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Detail">
                    <i class="material-symbols-outlined fs-16 fw-normal text-primary">visibility</i>
                  </a>

                  <a href="{{ route('admin.users.edit', $row->id) }}" class="bg-transparent p-0 border-0 hover-text-warning" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Edit">
